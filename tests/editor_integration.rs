...
#[tokio::test]
async fn test_update_quest_rejects_non_image_upload() {
    let pool = SqlitePool::connect("sqlite::memory:")
        .await
        .expect("Failed to create in-memory database");
    let db: Database = Database::with_pool(pool);
    ...
    let (bcast_tx, _) = broadcast::channel(128);
    let app_state = AppState::new(db, bcast_tx);
    let token = "test_session_upload_validation".to_string();
    app_state.create_session(token.clone()).await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"questTitle":"Quest","questDescription":null,"questExpValue":1,"questDayOfWeek":1,"questImage":[{"name":"note.txt","contents":"data:text/plain;base64,SGk=","type":"text/plain"}]}"#,
                ))
                .unwrap(),
        )
        .await
