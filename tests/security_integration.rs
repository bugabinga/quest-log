//! Integration tests for security: SQL injection prevention, XSS protection, path traversal.
#![allow(
...
async fn test_sql_injection_in_web_interface() {
    ...
    for (i, injection) in sql_injection_attempts.iter().enumerate() {
        ...
        assert!(
            ...
            "SQL injection attempt {i} should be handled safely"
        );
        ...
        assert!(
            ...
            "Database should remain intact after SQL injection attempt {i}"
        );
    }
...
async fn test_input_validation_comprehensive() {
    ...
    for (title, description) in test_inputs {
        ...
        assert!(
            result.is_ok(),
            "Should handle input safely: '{title}' - {description}"
        );
...
async fn test_path_traversal_prevention() {
    ...
    for attempt in path_traversal_attempts {
        ...
        assert_eq!(
            ...
            "Path traversal attempt should return 404: {attempt}"
        );
    }
...
async fn test_http_header_injection() {
    ...
    for (header_name, header_value) in header_injection_attempts {
        ...
        // Err(_) case: If request creation failed due to invalid header, that's good security
...
async fn test_dos_prevention_basic() {
    ...
    assert!(
        ...
        "Average response time should be reasonable: {avg_duration:?}"
    );

    println!(
        "Basic DoS prevention test completed successfully - avg response time: {avg_duration:?}"
    );
}
