#!/bin/bash
# Script to apply Editor documentation to ARCHITECTURE.md
# Run from the project root directory

set -e

ARCH_FILE="ARCHITECTURE.md"
BACKUP_FILE="ARCHITECTURE.md.backup"
...
# Create backup
cp "$ARCH_FILE" "$BACKUP_FILE"
echo "Created backup: $BACKUP_FILE"

# Find the line number after handlers.rs section (before database.rs)
INSERT_LINE=$(grep -n "### \`src/database.rs\`" "$ARCH_FILE" | cut -d: -f1 | head -1)
...
echo "Inserting Editor section at line $INSERT_LINE"

# Create the editor section
cat > /tmp/editor_insert.md << 'EDITOR_SECTION'
...
# Insert the editor section
head -n $((INSERT_LINE - 1)) "$ARCH_FILE" > /tmp/arch_new.md
cat /tmp/editor_insert.md >> /tmp/arch_new.md
tail -n +$INSERT_LINE "$ARCH_FILE" >> /tmp/arch_new.md

# Replace the original file
mv /tmp/arch_new.md "$ARCH_FILE"

echo "Successfully updated $ARCH_FILE"
echo ""
echo "You should also update these sections manually:"
echo "1. Add 'src/auth.rs' section to Code Map"
echo "2. Update 'src/state.rs' description"
echo "3. Update 'src/ui/' section with editor.rs and auth.rs"
echo "4. Update 'static/' section with editor.js"
echo "5. Update Environment Variables section"
echo "6. Add 'static/js/editor.js' to JavaScript Structure"
